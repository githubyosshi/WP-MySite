		...
		public function zakra_add_body_classes( $classes ) {
			...
			if ( is_singular( 'page' ) ) { // page.
				...
				if ( zakra_is_woocommerce_active() && ( is_checkout() || is_cart() || is_account_page() ) ) {
					$classes[] = get_theme_mod( 'zakra_structure_wc', 'tg-site-layout--right' );
					...
					$classes[] = get_theme_mod( 'zakra_structure_page', 'tg-site-layout--right' );
				}
			} elseif ( is_singular() ) { // Singular post.

				if ( zakra_is_woocommerce_active() && is_product() ) { // WC single product.
					$classes[] = get_theme_mod( 'zakra_structure_wc_product', 'tg-site-layout--right' );
				} elseif ( isset( $individual_layout ) && ! empty( $individual_layout ) && 'tg-site-layout--customizer' !== $individual_layout ) {
					$classes[] = $individual_layout;
				} else {
					$classes[] = get_theme_mod( 'zakra_structure_post', 'tg-site-layout--right' );
				}
				...
				if ( zakra_is_woocommerce_active() && is_woocommerce() ) {
					...
					$classes[] = get_theme_mod( 'zakra_structure_archive', 'tg-site-layout--right' );
				}
				...
				$classes[] = get_theme_mod( 'zakra_structure_default', 'tg-site-layout--right' );
			}
